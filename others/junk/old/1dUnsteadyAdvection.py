...
'''Undone, 1D unsteady Diffusion equation, implicit time, what is differen from v1 is that 
it is setup for D and C different than 1.'''
def sourceTerm(x, t, C):
...
for tt in range(1, timeStep):
    ...
    '''backward euler'''
    rhs[1:-1] = dt * sourceTerm(x[1:-1], t, C[1:-1]) + Q_old[1:-1]
    rhs[0] = q_exact(x[0], t)
    """right boundary dirichlet or neumann"""
    # rhs[-1] = q_exact(x[-1], t)
